...
#if NEUTRAL_AGE_SCREEN_MODULE
            if (config.NeutralAgeScreenConfig.AutoStart)
            {
...
#if NEUTRAL_AGE_SCREEN_MODULE
            var neutralAgeScreenModule = root.AddComponent<NeutralAgeScreenModule>();
            neutralAgeScreenModule.Initialize(config.NeutralAgeScreenConfig);
#endif
        }
    }
}
